...
public class RowFilteringObservationProviderTest {
	...
	@Before
	public void setUp() {
		provider = new ArrayObservationProvider<>(DATA, Observation::new);
	}
	...
	public void noRowFiltered() {
		...
		for (Integer[] row : DATA) {
			...
			assertArrayEquals(row, iterator.next().getFeatures(Integer[]::new));
		}
	...
	public void middleRowFiltered() {
		RowFilteringObservationProvider<Integer, Observation<Integer>> rfop =
				new RowFilteringObservationProvider<>(provider,
						observation -> observation.getFeatures(Integer[]::new)[1].equals(6));

		Iterator<Observation<Integer>> iterator = rfop.iterator();

		assertEquals(2, rfop.size());
		assertTrue(iterator.hasNext());

		assertArrayEquals(DATA[0], iterator.next().getFeatures(Integer[]::new));
		assertTrue(iterator.hasNext());
		assertArrayEquals(DATA[2], iterator.next().getFeatures(Integer[]::new));
		assertFalse(iterator.hasNext());
	}
	...
	public void lastRowFiltered() {
		RowFilteringObservationProvider<Integer, Observation<Integer>> rfop =
				new RowFilteringObservationProvider<>(provider,
						observation -> observation.getFeatures(Integer[]::new)[1].equals(0));
		...
		assertArrayEquals(DATA[0], iterator.next().getFeatures(Integer[]::new));
		assertTrue(iterator.hasNext());
		assertArrayEquals(DATA[1], iterator.next().getFeatures(Integer[]::new));
		assertFalse(iterator.hasNext());
	}
