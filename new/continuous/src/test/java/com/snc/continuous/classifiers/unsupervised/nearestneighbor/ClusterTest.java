...
public class ClusterTest {
	...
	public void removeObservation() {
		...
		Double[] features = observations.iterator().next().getFeatures(Double[]::new);
		assertEquals(observation[0], features[0], 0);
		assertEquals(observation[1], features[1], 0);
	}
	...
	private Observation<Double> createObservation(double[] data) {
		return new Observation<>(Arrays.stream(data).boxed().toArray(Double[]::new));
	}
}